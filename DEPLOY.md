...
Release `1.0.3` includes six English product screenshots under `bundle/listing/`, explicit saveable Agent session permission modes (`auto` and `fixed`), bounded hydrated game state, serialized Anna Storage writes, persistent Detective/accent selection feedback, complete custom interrogation replies, and browser regression coverage for the complete investigation loop plus reload recovery. Run `anna-app apps sync-meta --account $ANNA_HOST --json` after publishing if the listing does not immediately show the screenshot URLs.
...
Previously submitted Anna candidate (2026-08-31): version `1.0.2`, version id `609`,
...
URLs. `apps submit-review` pinned `review_candidate_version=1.0.2`; lifecycle was
`pending_review` / `is_published=false` until Anna approves it. For the new
candidate, install the exact `1.0.3` version before manual runtime verification,
then re-run `anna-app apps grants casefile-zero --account $ANNA_HOST --json`.
